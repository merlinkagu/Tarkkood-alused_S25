...
-   **`venue_cost`** saali rendi jaoks venue_cost \<- 100

-   **`food_cost_per_person`** toidu jaoks inimese kohta food_cost_per_person
    \<- 15

-   **`drink_cost_per_person`** jookide jaoks inimese kohta
    drink_cost_per_person \<- 5

-   **`entertainment_cost`** meelelahutuse jaoks entertainment cost \<- 50

-   **`number_of_people`** osalejate arv number_of_people \<- 10

Kasuta õpitud aritmeetilisi tehteid, et vastata järgmistele küsimustele
*(aritmeetika muutujatega)*:

1.  Kui kutsud 10 inimest, kui palju maksab toit? total_food_cost \<-
    food_cost_per_person \* number_of_people cat("Toidu maksumus 10 inimesega:",
    total_food_cost, "€\n")
...
```{r}
venue_cost <- 100                 # saali rent
food_cost_per_person <- 15        # toit inimese kohta
drink_cost_per_person <- 5        # joogid inimese kohta
entertainment_cost <- 50          # meelelahutus
number_of_people <- 10

food_total <- food_cost_per_person * number_of_people
drink_total <- drink_cost_per_person * number_of_people
total_cost <- venue_cost + food_total + drink_total + entertainment_cost
```

```{r}

total_party_cost <- function(number_of_people) { #number_of_people on osalejate arv
  venue_cost <- 100
  food_cost_per_person <- 15
  drink_cost_per_person <- 5
  entertainment_cost <- 50
  total <- venue_cost +
           (food_cost_per_person * number_of_people) +
           (drink_cost_per_person * number_of_people) +
           entertainment_cost
  
  return(total)
}

#proovin kas funktsioon töötab
total_party_cost(10)
...
#2
vanused_maatriks <- c(20, 21, 19, 22, 20, 23, 24, 22, 25)
punktid_maatriks <- c(85, 78, 90, 88, 76, 95, 87, 80, 92)
opilased_matrix <- matrix(c(vanused_maatriks, punktid_maatriks), ncol = 2)
colnames(opilased_matrix) <- c("Vanus", "Punktid")
opilased_matrix
...
```{r}
#3
nimed_koik <- c("Anna", "Mike", "Luis", "Sam", "Ella", "Ravi", "Mia", "Jack", "Olivia")
...
opilased_df <- data.frame(
  Nimi = nimed_koik,
  Vanus = vanused_maatriks,
  Punktid = punktid_maatriks,
  Klass = klassid
)
...
#5
korg_punktid <- subset(opilased_df, Punktid > 85, select = c(Nimi, Klass))
korg_punktid
```
...
Vastus: 20 erinevat argumenti on.
...
colnames(data_mem)
data_mem1 <- data_mem %>%
  mutate(Diff = Mem_Score_After - Mem_Score_Before)
head(data_mem1, 10)

# Üldine keskmine enne ja pärast
data_mem1 %>%
  summarise(
    mean_before = mean(Mem_Score_Before, na.rm = TRUE),
    mean_after  = mean(Mem_Score_After,  na.rm = TRUE))
```

```{r}
# 2

# Grupp Drug järgi: keskmine Diff + osalejate arv; sort kahanevalt
data_mem1 %>%
  group_by(Drug) %>%
  summarise(
...
# Grupp Drug + dosage; keskmine Diff; sort Drug sees Diff kahanevalt
data_mem1 %>%
...
# Tunnuse age_group loomine ja keskmiste arvutamine Drug x age_group lõikes
data_mem1 %>%
  mutate(
    age_group = case_when(
      age < 30 ~ "Noor",
  ...
  summarise(
    mean_diff = mean(Diff, na.rm = TRUE),
    n = n()) %>%
  arrange(age_group, desc(mean_diff))
```
...
data_mem1 %>%
  ...
  summarise(
    ...
    mean_diff   = mean(Diff, na.rm = TRUE),
    n = n()) %>%
  mutate(delta = mean_after - mean_before) %>%
  arrange(desc(mean_diff))
```
...
#juba varem on sisseloetud andmestik
